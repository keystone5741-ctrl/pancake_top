...
Phase 0에서 확정한 세계 규칙 (상세: `benchmarks/phase0-2026-09-17.md`):

- **시럽 규칙** — 팬케이크는 고정된 팬케이크나 바닥에 닿는 순간 그 자리에 정착한다. 미끄러지거나 튕기지 않는다.
- **드레이프 규칙** — 정착할 때 받침 면에 맞춰 눕는다. 받침의 기울기는 일부만 물려받아 기울기가 층마다 누적되지 않는다.
- **FROZEN 고정** — 묻힌 팬케이크는 공간에 고정된다. Collapse Day의 Release 명령 외에는 다시 움직이지 않는다.
...
**상태 (2026-09-17): 프로토타입 구현 및 1차 측정 완료.** `apps/physics-prototype`, 결과는 [`benchmarks/phase0-2026-09-17.md`](./benchmarks/phase0-2026-09-17.md).
핵심 결론: 자유 강체 물리로는 탑이 서지 않는다(높이 효율 3%). **시럽 규칙**(접촉 시 정착)과 **드레이프 규칙**(받침 면에 순응)이라는 두 개의 세계 규칙을 더해야 탑이 선다(효율 ~100%). 이 규칙들은 §7의 "PANCAKE DROP 세계의 규칙을 가진 물리"에 포함된다. 서버 시뮬레이션 비용은 팬케이크당 약 0.4 ms(단일 스레드)로 탑 크기와 무관하게 일정하다(FROZEN 구조 덕분). 실제 기기 FPS는 아직 미측정(Phase 1과 함께).

### Phase 0.5 — Physics & Visual Validation

새 기능 없이 Phase 0의 세계 규칙을 검증한다: 시각적 자연스러움, 침투 없는 안정성, 한 장씩 쌓이는 감각, 10분 Drop 안의 서버 계산 시간.

- Stacking 품질 계측(높이·수평 퍼짐·기울기·층 간격·침투)을 100 / 1k / 10k / 100k 에 대해 기록.
- Drape 프리셋 Stable / Natural / Loose 를 동일 seed·10k 로 비교하고 스크린샷으로 판단. 제품 후보는 수치가 아니라 가장 자연스러운 쪽.
- 100k Drop 이 T−60초 cutoff 안에 계산되는지 반복 측정 (p50 / p95 / 실패 / RSS).
- 서버 Transform 과 클라이언트 연출의 분리 검증 (연출 종료 시 서버 값에 정확히 수렴).
- 실제 기기 측정 절차(100k / 500k / 1M) 와 기기 보호(중단 후 마지막 정상 결과 기록) 준비.
