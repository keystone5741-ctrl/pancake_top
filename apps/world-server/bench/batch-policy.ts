...
for (const batch of batches) {
  ...
  const cfg = loadConfig({ databaseUrl: dbUrl, dataDir: dir, chunkSize: 10_000, simBatchSize: batch, simBatchWindowMs: 0, snapshotEveryPancakes: 1e9 });
  ...
  const r = {
    batch, count, wallSeconds: wall, purchaseSeconds: (tAlloc - t0) / 1000, throughputPerSec: count / wall, jobs: jm.length,
